...
#define DFLT_USRIMAGE_FILENAME "usrimage.jffs2"
#define DFLT_USRAPP_FILENAME   "usrapp.jffs2"
...
#define SPI_SECTOR_SIZE 4096  /*  4KB */
    ...
    #define ROM_SIZE (LOADER_SIZE + LOADER_BDINFO_SIZE + RTK_KERNEL_SIZE + UIMAGE1_SIZE + UIMAGE2_SIZE + USR_APP_SIZE + PARA_SIZE + SYSINFO_SIZE)
#else
...
#define UPGRADE_ACTION_USAGE \
    ...
    "    - FILENAME: Specify rom file name on remote TFTP server.\n" \
    "                If not specified, '" DFLT_ROM_FILENAME "' will be used\n"\
    "upgrade usrimage1 [FILENAME]\n" \
    "    - upgrade usrimage to first usrimage partition.\n" \
    "    - FILENAME: Specify rom file name on remote TFTP server.\n" \
    "                If not specified, '" DFLT_USRIMAGE_FILENAME "' will be used\n"\
    "upgrade usrimage2 [FILENAME]\n" \
    "    - upgrade usrimage to second usrimage partition.\n" \
    "    - FILENAME: Specify rom file name on remote TFTP server.\n" \
    "                If not specified, '" DFLT_USRIMAGE_FILENAME "' will be used\n"\
    "upgrade usrapp [FILENAME]\n" \
    "    - upgrade usrapp.\n" \
    "    - FILENAME: Specify rom file name on remote TFTP server.\n" \
    "                If not specified, '" DFLT_USRAPP_FILENAME "' will be used\n"
#else
#define UPGRADE_ACTION_USAGE \
    "loader [FILENAME]\n" \
    "    - upgrade loader with image validation, such as chip family, size and CRC.\n" \
...
int upgrade_loader (char *filename, int force)
{
	...
	unsigned long filesize = 0, filesizeRnd = 0;
	...
	filesize = simple_strtoul(fileSizeStr, NULL, 16);
	filesizeRnd = ROUND(filesize, SPI_SECTOR_SIZE);
    ...
    if (_write2flash(LOADER_START, filesizeRnd, 0))
    {
...
int upgrade_runtime (char *filename, int partition)
{
    ...
    /* Reset memory value to be the same as flash erased value */
    memset((void*)FILE_SOURCE_ADDR, 0xff, RTK_KERNEL_SIZE); /*check FILE_SOURCE_ADDR */
...
#endif
        if (_write2flash(RTK_KERNEL_START, filesizeRnd, 0))
        {
    ...
    else if (1 == partition)
    {
        ...
        if (_write2flash(KERNEL2_START, filesizeRnd, 0))
        {
            printf("Write runtime image [%s] %sfailed\n", filename, partitionStr);
            return 1;
        }
...
int upgrade_usrfile (char *filename, int partition)
{
    int ret = 0;
    char partitionStr[16];
	char *fileSizeStr = NULL;
    ...
    /* Reset memory value to be the same as flash erased value */
    memset((void*)FILE_SOURCE_ADDR, 0xff, UIMAGE1_SIZE); /*check FILE_SOURCE_ADDR */
	...
	filesize = simple_strtoul(fileSizeStr, NULL, 16);
	/* filesize is already round up to block size */
    ...
    if (0 == partition)
    {
        ...
        strcpy(partitionStr, "to partition 0 ");
#endif
        if (_write2flash(UIMAGE1_START, filesize, 0))
        {
            printf("Write usrimage1 [%s] %sfailed\n", filename, partitionStr);
            return 1;
        }
    }
//#ifdef CONFIG_DUAL_IMAGE
    ...
    {
        strcpy(partitionStr, "to partition 1 ");
        if (_write2flash(UIMAGE2_START, filesize, 0))
        {
            printf("Write usrimage2 [%s] %sfailed\n", filename, partitionStr);
            return 1;
        }
    }
//#endif
    ...
    {
        strcpy(partitionStr, "to partition 2 ");
        if (_write2flash(USR_APP_START, filesize, 0))
        {
            printf("Write usrapp [%s] %sfailed\n", filename, partitionStr);
            return 1;
        }
    }


    printf("Upgrade usr file [%s] %ssuccess\n", filename, partitionStr);
...
int upgrade_rom (char *filename, int force)
{
    ...
    if (!force)
    {
        ...
        offset = (LOADER_SIZE + LOADER_BDINFO_SIZE);
        if (_write2flash(RTK_KERNEL_START, (ROM_SIZE - offset), offset))
        {
...
int do_upgrade (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    ...
    else
    {
        ...
        else if (0 == strcmp(argv[1], "usrimage1"))
        {
            if (3 == argc)
                strncpy(filename, argv[2], sizeof(filename));
            else
                strcpy(filename, DFLT_USRIMAGE_FILENAME);

            upgrade_usrfile(filename, 0);
        }
        else if (0 == strcmp(argv[1], "usrimage2"))
        {
            if (3 == argc)
                strncpy(filename, argv[2], sizeof(filename));
            else
            ...
            upgrade_usrfile(filename, 1);
        }
        else if (0 == strcmp(argv[1], "usrapp"))
        {
            if (3 == argc)
                strncpy(filename, argv[2], sizeof(filename));
            else
                strcpy(filename, DFLT_USRAPP_FILENAME);

            upgrade_usrfile(filename, 2);
        }
