...
#define PARTITION_ADDR   CONFIG_SYS_FLASH_BASE + RTK_KERNEL_START
...
static int dual_image_sel (int index)
{
    ...
    if (rc)
    {
        ...
        {
            /* Erase image header when it is crash */
            eraseFlash(PARTITION_ADDR_1, 0x1000);
...
static int dual_image_sel (int index)
{
    ...
    memcpy(&ih, (const void *) PARTITION_ADDR, sizeof(ih));
    totalSize = ih.ih_size + sizeof(ih);
    if (totalSize <= RTK_KERNEL_SIZE)
    {
        memcpy((void *) RUNTIME_SRC_ADDR, (const void *) PARTITION_ADDR, totalSize);
        rc = run->cmd(run, 0, 2, argu);
    }
    else
    {
    ...
    if (rc)
    {
        printf ("## Boot from the partition is failed.\n");
    }
...
int do_boardid (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
//    int arg;
    int id = -1;
//    int rcode = 0;
