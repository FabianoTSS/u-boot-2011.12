...
void rtl8380_intGpio_DirSet(uint32 pin, drv_gpio_direction_t dir)
{
    uint32 val;
    uint32 offset;
	static uint32 gpio_a45_config=0;
	static uint32 gpio_b23456_config=0;
	...
	if (pin == GPIO_ID(GPIO_PORT_A, 4) || pin == GPIO_ID(GPIO_PORT_A, 5))
    {
        if (gpio_a45_config == 0)
        {
    ...
    if (pin == GPIO_ID(GPIO_PORT_B, 2) ||
        ...
        pin == GPIO_ID(GPIO_PORT_B, 4) ||
        pin == GPIO_ID(GPIO_PORT_B, 5) ||
        pin == GPIO_ID(GPIO_PORT_B, 6))
    {
        if (gpio_b23456_config == 0)
        {
            /*!<关闭GPIO10-14作为JTAG复用  */
            REG32(0xBB001000) &= ~(0x3 << 2);
            REG32(0xBB001000) |= (0x2 << 2);
            gpio_b23456_config = 1;
        }
    }
