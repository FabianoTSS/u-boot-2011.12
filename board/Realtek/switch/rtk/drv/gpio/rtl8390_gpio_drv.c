...
void rtl8390_intGpio_DirSet(uint32 pin, drv_gpio_direction_t dir)
{
    uint32 val;
    uint32 offset;
	static uint32 gpio_a34567_config=0;
	static uint32 gpio_b23_config=0;
	...
	if (pin == GPIO_ID(GPIO_PORT_A, 3) ||
        pin == GPIO_ID(GPIO_PORT_A, 4) ||
        pin == GPIO_ID(GPIO_PORT_A, 5) ||
        pin == GPIO_ID(GPIO_PORT_A, 6) ||
        pin == GPIO_ID(GPIO_PORT_A, 7))
    {
        if (gpio_a34567_config == 0)
        {
            REG32(0xBB000004) &= ~(0x3);
            REG32(0xBB000004) |= 0x2;
            gpio_a34567_config = 1;
        }
    }

	if (pin == GPIO_ID(GPIO_PORT_B, 2) ||
	    pin == GPIO_ID(GPIO_PORT_B, 3))
	{
		if (gpio_b23_config == 0)
		{
			REG32(0xBB000004) |= (0x1 << 7);
			gpio_b23_config = 1;
		}
	}
