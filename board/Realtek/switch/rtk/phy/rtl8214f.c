...
/* Function Name:
 *      rtl8214fb_linkDownPowerSavingEnable_set
 * Description:
 ...
 *      < 0 - Fail.
 * Note:
 *      For RTL8214B/RTL8214FB/RTL8212B.
 */
int rtl8214fb_linkDownPowerSavingEnable_set(int port, int enable)
{
    uint32  phyData;
    ...
    if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_STATUS_REG, &val)) != 0)
        return ret;

    if (val & LinkStatus_MASK)
    {
        if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, 15, &phyExtStatus15)) != 0)
            return ret;
        ...
        {
            /* get value from CHIP*/
            if ((ret = gMacDrv->drv_miim_read(port, 0, 21, &phyData)) != 0)
                return ret;
            ...
            if ((ret = gMacDrv->drv_miim_write(port, 0, 21, phyData)) != 0)
                return ret;
    ...
    if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_8, 30, &restore_val)) != 0)
        return ret;
    ...
    if ((restore_val & 0x2000) == 0x0000)
    {
        ...
        if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 30, val)) != 0)
            return ret;
    }
    ...
    if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 30, restore_val)) != 0)
        return ret;
 ...
 *      For RTL8214B/RTL8214FB/RTL8212B.
 */
int rtl8214fb_greenEnable_set(int port, int enable)
{
    uint32  phyData;
    int32   ret = -1;
    ...
    if ((ret = gMacDrv->drv_miim_read(port, 5, 1, &phyData)) != 0)
        return ret;
    phyData &= 0xFEFF;
    if (ENABLE == enable)
        phyData = 0x0201;
        ...
        phyData = 0x0301;
    if ((ret = gMacDrv->drv_miim_write(port, 5, 1, phyData)) != 0)
        return ret;
    /* setting the GreenEthernet TX/RX Threshold */
    if (ENABLE == enable)
    {
        /* enable RX GreenEthernet threshold */
        if ((ret = gMacDrv->drv_miim_write(port, 3, 24, 0xBC3D)) != 0)
            return ret;
        /* enable TX GreenEthernet threshold */
        if ((ret = gMacDrv->drv_miim_write(port, 3, 25, 0x3C3D)) != 0)
            return ret;
    }
    else
    {
        /* disable RX GreenEthernet threshold */
        if ((ret = gMacDrv->drv_miim_write(port, 3, 24, 0xBC3C)) != 0)
            return ret;
        /* disable TX GreenEthernet threshold */
        if ((ret = gMacDrv->drv_miim_write(port, 3, 25, 0x3C3C)) != 0)
            return ret;
    }
...
int rtl8214fb_enable_set(int port, int enable)
{
    ...
    if ((restore_val & 0x2000) == 0x0000)
    {
        ...
        {   /* Original media: Auto-Fiber */
            ...
            if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 30, val)) != 0)
                return ret;
            if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_CONTROL_REG, &phyData_copper)) != 0)
                return ret;
            phyData_copper &= ~(PowerDown_MASK);
            if (DISABLE == enable)
                phyData_copper |= (1 << PowerDown_OFFSET);
            if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_0, PHY_CONTROL_REG, phyData_copper)) != 0)
                return ret;

            val |= (0x1000);
            if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 30, val)) != 0)
                return ret;
            if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_CONTROL_REG, &phyData_fiber)) != 0)
                return ret;
            phyData_fiber &= ~(PowerDown_MASK);
            if (DISABLE == enable)
                phyData_fiber |= (1 << PowerDown_OFFSET);
            if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_0, PHY_CONTROL_REG, phyData_fiber)) != 0)
                return ret;
        }
        else
    ...
    {
        if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_CONTROL_REG, &phyData)) != 0)
            return ret;
        ...
        {   /* force-fiber media */
            ...
            {
                /* Force TX 2.5G */
                if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 31, 0x000F)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 30, 0x0018)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 25, 0x374D)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 31, 0x0008)) != 0)
                    return ret;
                /* force power */
                if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_8, 31, 0x000F)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 30, 0x001E)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 21, 0xC129)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 31, 0x0008)) != 0)
                    return ret;
            }
            else
            {
                ...
                if ((ret = gMacDrv->drv_miim_write(port, 15, 30, 0x0018)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 25, 0x074D)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 31, 0x0008)) != 0)
                    return ret;
                /* Default setting */
                ...
                if ((ret = gMacDrv->drv_miim_write(port, 15, 30, 0x001E)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 21, 0xC178)) != 0)
                    return ret;
                if ((ret = gMacDrv->drv_miim_write(port, 15, 31, 0x0008)) != 0)
                    return ret;
            }
        ...
        if ((ret = gMacDrv->drv_miim_write(port, PHY_PAGE_0, PHY_CONTROL_REG, phyData)) != 0)
            return ret;
    }
    OSAL_UDELAY(500000);

    return 0;
} /* end of rtl8214fb_enable_set */


/* Function Name:
 *      rtl8214fb_crossOverMode_set
 * Description:
 ...
 *      None.
 */
int rtl8214fb_crossOverMode_set(int port, int mode)
{
    int32   ret;
    ...
    if (rtk_phyBaseMacIdFromMacId(port, (int*)&base_port) < 0)
    {
        printf("Unable to find base MAC ID of the PHY!\n");
        return -1;
    }
    ...
    ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_STATUS_REG, &val);
    if ((ret = gMacDrv->drv_miim_read(port, PHY_PAGE_0, PHY_STATUS_REG, &val)) != 0)
        return ret;
    ...
    if (val & LinkStatus_MASK)
    {
        ...
        if (is_copper_linkup)
        {
        	/*Power Down*/
			portEnable = DISABLE;
			rtl8214fb_enable_set(port, portEnable);


			OSAL_MDELAY(1000);

		    if ((ret = gMacDrv->drv_miim_read(base_port+1, 8, 16, &restore_phyData)) != 0)
		        goto set_crossOver_failed;

		    phyData = restore_phyData & ~(1<<(12+port-base_port));
		    phyData |= (1<<(8+port-base_port));

		    if ((ret = gMacDrv->drv_miim_write(base_port+1, 8, 16, phyData)) != 0)
		        goto set_crossOver_failed;

            /* get value from CHIP*/
            if ((ret = gMacDrv->drv_miim_write(port, 7, 0x1e, 0x002d)) != 0)
                goto set_crossOver_failed;
            if ((ret = gMacDrv->drv_miim_read(port, 7, 0x18, &phyData)) != 0)
                goto set_crossOver_failed;
            phyData &= ~(1 << 5);
            phyData |= (force_mode << 5);
            if ((ret = gMacDrv->drv_miim_write(port, 7, 0x18, phyData)) != 0)
                goto set_crossOver_failed;

            if ((ret = gMacDrv->drv_miim_read(port, 0, 0x10, &phyData)) != 0)
                goto set_crossOver_failed;
            phyData &= ~(1 << 5);
            phyData |= (mdi << 5);
            if ((ret = gMacDrv->drv_miim_write(port, 0, 0x10, phyData)) != 0)
                goto set_crossOver_failed;
set_crossOver_failed:
		    if ((ret = gMacDrv->drv_miim_write(base_port+1, 8, 16, restore_phyData)) != 0)
        		return ret;
			/*Power Up*/
			portEnable = ENABLE;
			rtl8214fb_enable_set(port, portEnable);
    ...
    return ret;
} /* end of rtl8214fb_crossOverMode_set */
