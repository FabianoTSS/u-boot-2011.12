...
static void rtl8380_mac_config_init(const rtk_switch_model_t *pModel)
{
    ...
    /* Combo Port LED Patch */
    MEM32_WRITE(0xbb00005c, 0x1b);

    MEM32_WRITE(0xbb00a17c, 0xfffffff);

    OSAL_MDELAY(50);

    MEM32_WRITE(0xbb00a17c, 0);
    ...
    if (LED_IF_SEL_NONE != pModel->led.led_if_sel)
    {
        ...
        MEM32_WRITE_FIELD(SWCORE_BASE_ADDR| RTL8380_LED_MODE_SEL_ADDR,
                RTL8380_LED_MODE_SEL_LED_MODE_SEL_OFFSET,
                RTL8380_LED_MODE_SEL_LED_MODE_SEL_MASK, i);
    }
#endif
    ...
    if (board_model && ((0 == strcmp(board_model, "RTL8330M_INTPHY_8212B_SGMII_DEMO"))))
    {
    	OSAL_PRINTF("Board model: RTL8330M_INTPHY_8212B_SGMII_DEMO!\n");
	MEM32_WRITE(0xbb00a1d8, 0xDE758);
    }
    else
    {
    ...
    for (phy_idx=0; phy_idx<pModel->phy.count; phy_idx++)
    {
        switch (pModel->phy.list[phy_idx].chip)
        {
            ...
            case RTK_CHIP_RTL8214C:
                {
                    Tuint8 rtl8214c_phy0_macid = pModel->phy.list[phy_idx].mac_id - pModel->phy.baseid;
                    rtl8214c_config(rtl8214c_phy0_macid);
                }
                break;
            #endif
            ...
            case RTK_CHIP_RTL8214QF:
                {
                    Tuint8 rtl8214qf_phy_macid = pModel->phy.list[phy_idx].mac_id - pModel->phy.baseid;
                    rtl8214qf_config(rtl8214qf_phy_macid);
                }
                break;
            ...
            case RTK_CHIP_RTL8218D:
                {
                    Tuint8 rtl8218d_phy_macid = pModel->phy.list[phy_idx].mac_id - pModel->phy.baseid;
                    rtl8218d_config(MY_UNIT, rtl8218d_phy_macid);
                }
            break;
            #endif
...
static rtl8380_reg_value_t rtl8380_serdes01_qsgmii_6275b_8218d[] = {
  {0xbb000ff8, 0xaaaaaaaf},{0xbb00f38c, 0x0000f46f},
  {0xbb00f388, 0x000085fa},{0xbb00f488, 0x000085fa},{0xbb00f398, 0x000020d8},{0xbb00f498, 0x000020d8},
  {0xbb00f3c4, 0x0000B7C9},{0xbb00f4ac, 0x00000482},{0xbb00f4a8, 0x000080c7},{0xbb00f3c8, 0x0000ab8e},
  {0xbb00f3ac, 0x00000482},{0xbb00f3cc, 0x000024ab},{0xbb00f4c4, 0x00004208},{0xbb00f4c8, 0x0000c208},
  {0xbb00f464, 0x0000b403},{0xbb00f564, 0x0000b403},{0xbb00f3b8, 0x0000FCC2},{0xbb00f4b8, 0x0000FCC2},
  {0xbb00f3a4, 0x00008e64},{0xbb00f3a4, 0x00008c64},{0xbb00f4a4, 0x00008e64},{0xbb00f4a4, 0x00008c64},
  };
...
static rtl8380_reg_value_t rtl8380_serdes23_qsgmii_6275b_8218d[] = {
  {0xbb00f58c, 0x0000f46d},
  {0xbb00f588, 0x000085fa},{0xbb00f688, 0x000085fa},{0xbb00f788, 0x000085fa},{0xbb00f598, 0x000020d8},
  {0xbb00f698, 0x000020d8},{0xbb00f5c4, 0x0000B7C9},{0xbb00f5c8, 0x0000ab8e},{0xbb00f5ac, 0x00000482},
  {0xbb00f6ac, 0x00000482},{0xbb00f5cc, 0x000024ab},{0xbb00f6c4, 0x00004208},{0xbb00f6c8, 0x0000c208},
  {0xbb00f664, 0x0000b403},{0xbb00f764, 0x0000b403},{0xbb00f5b8, 0x0000FCC2},{0xbb00f6b8, 0x0000FCC2},
  {0xbb00f5a4, 0x00008e64},{0xbb00f5a4, 0x00008c64},{0xbb00f6a4, 0x00008e64},{0xbb00f6a4, 0x00008c64},
};
...
static rtl8380_reg_value_t rtl8380_serdes4_qsgmii_8214qf_6275b[] = {\
  {0xbb00f78c, 0x0000f46d},{0xbb00f788, 0x000085fa},{0xbb00f7ac, 0x00000482},{0xbb00f798, 0x000020d8},\
  {0xbb00f7a8, 0x000058c7},{0xbb00f7c4, 0x0000B7C9},{0xbb00f7c8, 0x0000ab8e},{0xbb00f864, 0x00000003},\
  {0xbb00f7b8, 0x0000FCC2},{0xbb00f7a4, 0x00008e64},{0xbb00f7a4, 0x00008c64},\
};
...
static void rtl8380_intraSerdes_config_init(const rtk_switch_model_t *pModel)
{
    ...
    else
    {
        ...
        else if(pModel->serdes.list[0].mii == RTK_MII_QSGMII)
        {
            ...
            if(pModel->phy.list[pModel->serdes.list[0].phy_idx].chip == RTK_CHIP_RTL8218D)
            {
                /*Save 0xbb000FF8 bit0-bit3 from Array*/
                value_temp = rtl8380_serdes01_qsgmii_6275b_8218d[0].val;
                value_temp &= 0x0000000F;

                /*Reconstruct data and write back into array*/
                value |= value_temp;
                rtl8380_serdes01_qsgmii_6275b_8218d[0].val = value;

                for (i=0; i<(sizeof(rtl8380_serdes01_qsgmii_6275b_8218d)/sizeof(rtl8380_reg_value_t)); i++)
                {
                    MEM32_WRITE(rtl8380_serdes01_qsgmii_6275b_8218d[i].reg,  rtl8380_serdes01_qsgmii_6275b_8218d[i].val);
                    OSAL_MDELAY(1);
                }
            }
            else
        ...
        {
            if(pModel->phy.list[pModel->serdes.list[2].phy_idx].chip == RTK_CHIP_RTL8218D)
            {
                for (i=0; i<(sizeof(rtl8380_serdes23_qsgmii_6275b_8218d)/sizeof(rtl8380_reg_value_t)); i++)
                {
                    MEM32_WRITE(rtl8380_serdes23_qsgmii_6275b_8218d[i].reg,  rtl8380_serdes23_qsgmii_6275b_8218d[i].val);
                    OSAL_MDELAY(1);
                }
            }
            else
        ...
        else if(pModel->serdes.list[4].mii == RTK_MII_1000BX_FIBER || pModel->serdes.list[4].mii == RTK_MII_SGMII)
        {
        ...
        {
            if(pModel->phy.list[pModel->serdes.list[4].phy_idx].chip == RTK_CHIP_RTL8214QF)
            {
                for (i=0; i<(sizeof(rtl8380_serdes4_qsgmii_8214qf_6275b)/sizeof(rtl8380_reg_value_t)); i++)
                {
                    MEM32_WRITE(rtl8380_serdes4_qsgmii_8214qf_6275b[i].reg,  rtl8380_serdes4_qsgmii_8214qf_6275b[i].val);
                    OSAL_MDELAY(1);
                }
            }
            else
        ...
        else if(pModel->serdes.list[5].mii == RTK_MII_1000BX_FIBER || pModel->serdes.list[5].mii == RTK_MII_SGMII)
        {
 ...
 static  void rtl8380_enable_polling_phy(const rtk_switch_model_t *pModel)
{
    ...
    if (pModel && ((0 == strcmp(pModel->name, "RTL8381M_INTPHY_8214FC_2FIB_1G_DEMO"))))
    {
        val = 0x050FFF00;
        MEM32_WRITE(0xbb00a17c, val);
    }

    if (pModel && ((0 == strcmp(pModel->name, "RTL8381M_INTPHY_8214C_2FIB_1G_DEMO"))))
    {
        val = 0x050FFF00;
        MEM32_WRITE(0xbb00a17c, val);
    }
...
void rtl8231_init(void)
{
    ...
    /*Select GPIO*/
    extGpio_reg_write(0, RTL8231_PIN_SEL_REG, 0xffff);
    extGpio_reg_write(0, RTL8231_PIN_SEL_REG+1, 0xffff);


    /*Configure all 8231 GPIO to GPI*/
    /* reg 0x5(gpio 15~0) */
    val = 0xffff;
    extGpio_reg_write(0, 0x5, val);

    /* reg 0x6(gpio 31~16) */
    val = 0xffff;
    extGpio_reg_write(0, 0x6, val);

    /* reg 0x4[9:5] (gpio 36~32) */
    extGpio_reg_read(0, 0x4, &val);
    val |= 0x1f << 5;
    extGpio_reg_write(0, 0x4, val);

    /*Set 8231 PIN5/6/8 output*/
    extGpio_pin_dir_set(0 , EXT_GPIO_ID5,  EXT_GPIO_DIR_OUT);
    extGpio_pin_dir_set(0 , EXT_GPIO_ID6,  EXT_GPIO_DIR_OUT);
    extGpio_pin_dir_set(0 , EXT_GPIO_ID8,  EXT_GPIO_DIR_OUT);

    /*Set 8231 PIN5/6/8 output high*/
    extGpio_pin_data_set(0 , EXT_GPIO_ID5,  0x1);
    extGpio_pin_data_set(0 , EXT_GPIO_ID6,  0x1);
    extGpio_pin_data_set(0 , EXT_GPIO_ID8,  0x1);

    /*Enable 8231*/
    extGpio_dev_enable(0 ,  0x1);
}
#endif
