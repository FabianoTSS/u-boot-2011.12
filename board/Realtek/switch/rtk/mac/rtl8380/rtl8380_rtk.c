...
{
    printf("Force port28 link up 1G\n");
...
int32 rtk_htp_detect(void)
{
    ...
    uint32 htp_gpio_val=0,htp_gpio_val_sum=0;

    rtl8231_pin_direction_set(RTL8231_PIN_7,rtl8231_PIN_DIR_IN);

    OSAL_MDELAY(500);

    for(round=0;round<RTL8231_GPIO_READ_TIMES;round++)
    {
        rtl8231_pin_data_get(RTL8231_PIN_7,&htp_gpio_val);
...
int32 rtk_rstDeftGpio_detect(void)
{
    ...
    uint32 rst_gpio_detect_0_sum,rst_gpio_detect_1_sum;

    rst_gpio_detect_0_sum = 0;
    rst_gpio_detect_1_sum = 0;
    rtl8231_pin_direction_set(RTL8231_PIN_3,rtl8231_PIN_DIR_IN);

    rtl8231_pin_data_get(RTL8231_PIN_3,&rst_gpio_val);
#if 0
    if((0 == rst_gpio_val)&&(0 == rst_gpio_detect_0_sum))
    {
        rst_gpio_detect_0_sum = 1;
    }

    if((1 == rst_gpio_val)&&
        (0 == rst_gpio_detect_1_sum)&&
        (1 == rst_gpio_detect_0_sum))
    {
        rst_gpio_detect_1_sum = 1;
    }

    if((1 == rst_gpio_detect_0_sum)&&(1 == rst_gpio_detect_1_sum))
    {
        rst_gpio_detect_0_sum = 0;
        rst_gpio_detect_1_sum = 0;
