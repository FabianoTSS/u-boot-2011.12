...
static int swnic_init(struct eth_device *dev, bd_t *bis)
{
    ...
    REG32(SWCORE_BASE_ADDR | RTL8380_MAC_PORT_CTRL_ADDR(CPU_PORT)) = 0x0;
    ...
    REG32(SWCORE_BASE_ADDR | RTL8380_MAC_PORT_CTRL_ADDR(CPU_PORT)) = 0x3;
    ...
    *pTxRing[0] = ((unsigned int)pTxPkthdr & 0x0fffffff) | WRAP;
    ...
    REG32(SWCORE_BASE_ADDR | RTL8380_DMA_IF_INTR_MSK_ADDR) = 0xfffff;
...
static int swnic_recv(struct eth_device *dev)
{
    ...
    if (((*pRxRing[0]) & SWITCH_OWN) == 0x0) {
        ...
        if ((uint8*)((uint32)pRxPkthdr->buf_addr | 0xa0000000) >= (pRxCluster + (SWNIC_CLUSTER_SIZE * SWNIC_RX_CLUSTER_NUM)))
        {
            pRxPkthdr->buf_addr = (uint8 *)((uint32)pRxCluster & 0x0fffffff);
        }
